  ...
  for(int i = 0; i < x_width-1; i++)
  {
    for(int j = 0; j < y_height-1; j++)
    {
      for(int k = 0; k < z_depth-1; k++)
      {
        ...
        int v0 = field_states  [i]    [j]    [k];
        int v1 = field_states  [i+1]  [j]    [k];
        int v2 = field_states  [i+1]  [j]    [k+1];
        int v3 = field_states  [i]    [j]    [k+1];
        int v4 = field_states  [i]    [j+1]  [k];
        int v5 = field_states  [i+1]  [j+1]  [k];
        int v6 = field_states  [i+1]  [j+1]  [k+1];
        int v7 = field_states  [i]    [j+1]  [k+1];
        
        
        PVector v0Pos = new PVector(i*res,       k*res,        j*res);
        PVector v1Pos = new PVector((i+1)*res,   k*res,        j*res);
        PVector v2Pos = new PVector((i+1)*res,   (k+1)*res,    j*res);
        PVector v3Pos = new PVector(i*res,       (k+1)*res,    j*res);
        PVector v4Pos = new PVector(i*res,       k*res,        (j+1)*res);
        PVector v5Pos = new PVector((i+1)*res,   k*res,        (j+1)*res);
        PVector v6Pos = new PVector((i+1)*res,   (k+1)*res,    (j+1)*res);
        PVector v7Pos = new PVector(i*res,       (k+1)*res,    (j+1)*res);
        // fill array
